...
SgFile::RunFrontend()
{
  ...
  if (KEEP_GOING_CAUGHT_FILE_FRONTEND_SIGNAL == false)
  {
      try
      {
          //-----------------------------------------------------------
  ...
  else
  {
      std::cout
          ...
          << "SgFile::RunFrontend() signal"
          << std::endl;

      this->set_frontendErrorCode(1);
      status_of_function = -1;
  }
...
SgProject::create_files()
{
  ...
  BOOST_FOREACH(std::string filename, all_filenames)
  {
      ...
      ROSE_ASSERT (file->get_parent() != NULL);
      ROSE_ASSERT (file->get_parent() != NULL);
      ROSE_ASSERT (file->get_parent() == this);
      ROSE_ASSERT (isSgProject(file->get_parent()) != NULL);
...
SgProject::RunFrontend()
{
  ...
  std::vector<SgFile*> all_files = get_fileList();
  {
      ...
      {
          status_of_file = file->callFrontEnd();
          status_of_function =
